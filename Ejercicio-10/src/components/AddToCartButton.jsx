  ...
  const handleAdd = (e) => {
    ...
    if (
      ...
      Swal.fire({
        icon: "info",
        title: "Tenes que...",
        text: "Seleccioná color y talle antes de agregar al carrito.",
      });
    ...
    if (existingItem) {
      ...
      Swal.fire({
        icon: "success",
        title: "Agregaste...",
        text: "otra unidad del mismo producto",
      });
...
return (
  <button
    className={`btn ${isInCart ? "btn-secondary" : "btn-primary"} rounded-pill ${className}`}
    onClick={!isInCart ? handleAdd : undefined}
    disabled={isInCart}
  >
    {isInCart ? "Ya está el carrito" : "Agregar al carrito"}
  </button>
);
